 ...
 * zero degree-of-freedom (i.e., welded) joint. It includes
 * "retractable" legs that allow the cat to change the effective
 * moment of inertia of each of its halves.
 * */

class RetractLegsModeling : public FelisCatusModeling
{
    void addJoints();
    void addActuators();
};
...
{
    RetractLegsModeling m = RetractLegsModeling();
    m.makeModel("Leland_retractLegs",
            "feliscatus_retractLegs.osim");

    return EXIT_SUCCESS;
};

void RetractLegsModeling::addJoints()
{
    ...
    WeldJoint * anteriorPosterior = new WeldJoint("anterior_posterior",
...
void RetractLegsModeling::addActuators()
{
