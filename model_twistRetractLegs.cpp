 ...
 * one degree-of-freedom (spinal twist) joint. It also includes 
 * "retractable" legs that allow the cat to change the effective
 * moment of inertia of each of its halves.
 * */

class TwistRetractLegsModeling : public FelisCatusModeling
{
    void addJoints();
    void addActuators();
};
...
{
    TwistRetractLegsModeling m = TwistRetractLegsModeling();
    m.makeModel("Leland_twistRetractLegs",
            "feliscatus_twistRetractLegs.osim");

    return EXIT_SUCCESS;
};

void TwistRetractLegsModeling::addJoints()
{
...
void TwistRetractLegsModeling::addActuators()
{
